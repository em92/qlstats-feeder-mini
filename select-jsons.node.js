...
function processGameData(file, data) {
  ...
  if (",ffa,ca,duel,tdm,ctf,ft,".indexOf(gt) < 0) {
    console.log(file + "\t" + gt + "\t" + data.gameEndTimestamp + "\taborted=" + data.matchStats.ABORTED + "\tplayers=" + data.playerStats.length);
  }
  return true;
}
...
function main() {
  ...
  processJsonFiles([_config.feeder.jsondir + "/2016-01/06/"])
