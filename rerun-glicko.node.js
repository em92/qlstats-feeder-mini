  ...
  return extractDataFromJsonFile(file)
    .then(function(result) {
      ...
      for (var i = 0; i < playerRanking.length; i++) {
        ...
        //if (gameId == 863)
        //  console.log(p1.name + ": r=" + p1.rating.__oldR + ", rd=" + p1.rating.__oldRd + ", g=" + p1.games + ", s=" + p1.score);
...
function extractDataFromJsonFile(path) {
  return Q
    ...
    .then(function(json) { return extractDataFromGameObject(JSON.parse(json)); });
}
...
function saveResults(cli, players) {
  ...
  for (var steamId in players) {
    ...
    if (player.games && player.mustSave)
      list.push(player);
  }
